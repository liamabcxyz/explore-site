...
// Same cut visibilityCategory uses internally for "blocked," exposed so a
// caller checking multiple sample points against just this one question
// (e.g. computeRooftopLayer.js's large-building multi-sample check) doesn't
// need to redeclare the threshold or run the full angle/comfort logic to
// get an answer it isn't asking for.
export function isBlocked(frac) {
  return frac < BLOCKED_FRAC_THRESHOLD;
}
