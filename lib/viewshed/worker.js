...
import { unpackViewshedGrid, unpackRooftopLayer } from "@/lib/viewshed/wasmUnmarshaling";
...
//
// Post-C6: `impl=wasm` now actually returns WASM-derived
// FeatureCollections (rebuilt from the flat Float64Array outputs by
// wasmUnmarshaling). Three lanes:
//   - impl=js (default): synchronous JS-only path — fastest first-byte,
//     no WASM load required
//   - impl=wasm: async WASM-only path. WASM's grid + rooftop outputs
//     get unpacked into the same FeatureCollection shape JS produces
//     and shipped as the sole result. If the WASM module fails to
//     load (e.g. asyncWebAssembly not supported), we silently fall
//     back to JS + warn once
//   - impl=both: async JS + WASM. JS is canonical (rendered), WASM is
//     diffed cell-by-cell / building-by-building and logged for
//     divergence.
//
// The reason JS stays canonical under impl=both — not just for
// dev/debug ergonomics — is that a subtle WASM regression that only
// flips a handful of cells wouldn't be caught by the render but would
// show up in the diff log; opting into `impl=wasm` explicitly
// signals "I trust the WASM output, don't run JS."
...
/**
 * Run the two WASM entries (grid + rooftop) on the same args and
 * return both flat outputs plus per-call timing. Packing is amortized
 * across the two — one packBuildings + one packTerrainGrid call.
 */
async function runWasmOnly(args) {
  const wasm = await loadWasm();
  ...
  return { gridFlat, roofFlat, packMs, gridComputeMs, roofComputeMs };
}

// One-time fallback warning per session — if WASM fails to load and
// we drop back to JS, tell the user in the console. Don't spam.
let warnedWasmFallback = false;
function warnWasmFallback(reason) {
  if (warnedWasmFallback) return;
  warnedWasmFallback = true;
  console.warn(
    `[viewshed impl] WASM path failed (${reason}) — falling back to JS. ` +
    "Retry after reload, or force JS with ?impl=js."
  );
}
...
self.onmessage = async (event) => {
  try {
    ...
    // Fast synchronous path — the default and, until a user opts into
    // ?impl=wasm, the only path with production traffic.
    if (impl === "js") {
      const grid = computeViewshed(args);
      const rooftop = computeRooftopLayer(args);
      self.postMessage({ grid, rooftop, impl });
      return;
    }

    // impl=wasm: WASM only — no JS reference is run, since the whole
    // point of opting in is to trust the WASM output. Silent fallback
    // to JS with a warning if the WASM load fails (dev env without
    // asyncWebAssembly, browser missing WebAssembly, etc.).
    if (impl === "wasm") {
      try {
        const w = await runWasmOnly(args);
        const grid = unpackViewshedGrid(
          w.gridFlat,
          args.launch,
          args.analysisRadius,
          args.radialSpacing ?? 20,
        ...
        const rooftop = unpackRooftopLayer(w.roofFlat, args.buildings);
        self.postMessage({ grid, rooftop, impl });
        const bldgCount = (args.buildings || []).length;
        const totalMs = w.gridComputeMs + w.roofComputeMs + w.packMs;
        console.log(
          `[viewshed perf] impl=wasm | ${bldgCount} bldgs | ` +
            `grid ${w.gridComputeMs.toFixed(1)}ms + rooftop ${w.roofComputeMs.toFixed(1)}ms ` +
            `(pack ${w.packMs.toFixed(1)}ms) = ${totalMs.toFixed(1)}ms`,
        );
      } catch (err) {
        warnWasmFallback(String(err && err.message || err));
        const grid = computeViewshed(args);
        const rooftop = computeRooftopLayer(args);
        self.postMessage({ grid, rooftop, impl: "js" });
      }
      return;
    }

    // impl=both: JS canonical, WASM diffed. JS ships first so the UI
    // isn't blocked on the WASM load — the WASM comparison log is a
    // side-channel via console.
    const jsGridStart = performance.now();
    const jsGrid = computeViewshed(args);
    const jsGridMs = performance.now() - jsGridStart;
    const jsRoofStart = performance.now();
    const jsRooftop = computeRooftopLayer(args);
    const jsRoofMs = performance.now() - jsRoofStart;
    self.postMessage({ grid: jsGrid, rooftop: jsRooftop, impl });

    runWasmCompute(args, jsGrid, jsRooftop)
      .then((w) => {
        if (w == null) return;
        ...
        const gridSpeedup = w.grid.computeMs > 0 ? jsGridMs / w.grid.computeMs : Infinity;
        const roofSpeedup = w.roof.computeMs > 0 ? jsRoofMs / w.roof.computeMs : Infinity;
        const bldgCount = (args.buildings || []).length;
        const totalJs = jsGridMs + jsRoofMs;
        ...
        console.log(
          `[viewshed perf] ${bldgCount} bldgs, ${w.grid.cells} cells | ` +
            `JS grid ${jsGridMs.toFixed(1)}ms + rooftop ${jsRoofMs.toFixed(1)}ms | ` +
