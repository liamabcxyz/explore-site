...
//
// Currently the Rust port of computeViewshed / computeRooftopLayer
// doesn't exist yet (that's C4/C5). `wasm` and `both` therefore fall
// back to `js` with a one-time console warning per session — the
// scaffolding is here so C4 lands as a targeted edit at the two
// TODO markers below, not a fresh integration.
let warnedWasmMissing = false;
function warnWasmMissing(impl) {
  if (warnedWasmMissing) return;
  warnedWasmMissing = true;
  console.warn(
    `[viewshed impl] "${impl}" requested but Rust port not landed yet — falling back to JS. ` +
    `Watch for phase C4 to fill in the WASM path.`
  );
}
...
self.onmessage = (event) => {
  try {
    ...
    // JS path — canonical today, kept as reference forever after C4/C5
    // so `impl=both` can diff against it.
    const runJs = () => ({
      grid: computeViewshed(args),
      rooftop: computeRooftopLayer(args),
    });

    // TODO(C4): once compute_viewshed / compute_rooftop_layer exist in
    // vantage-core, replace this stub with an async-import call that
    // packs args via wasmMarshaling and unpacks the returned typed
    // arrays into the same {grid, rooftop} shape runJs produces.
    const runWasm = () => {
      warnWasmMissing(impl);
      return runJs();
    };
    ...
    if (impl === "js") {
      ...
      // "both" — run both, diff, return whichever the user's actively
      // watching. During C4 development that's WASM; today they're the
      // same object because runWasm falls back to runJs. TODO(C4):
      // add a cell-by-cell diff and log divergences.
      const jsResult = runJs();
      const wasmResult = runWasm();
      result = wasmResult; // gets replaced with a real cross-check in C4
      // Silence "unused" lints on jsResult until the diff step lands.
      void jsResult;
    }

    self.postMessage({ ...result, impl });
  } catch (err) {
