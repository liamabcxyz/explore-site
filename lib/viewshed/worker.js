...
async function wasmSelfCheck() {
  try {
    ...
    const header = anyDivergence
      ? "[C2 wasm] SELF-CHECK FAILED — see rows"
      : `[C2 wasm] self-check: bit-identical across ${Object.keys(tally).length} functions, seed=0x9E3779B9`;
    (anyDivergence ? console.error : console.log)([header, ...rows].join("\n"));
  } catch (err) {
    console.error("[C2 wasm] load failed:", err);
  }
}
wasmSelfCheck();
