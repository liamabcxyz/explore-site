    ...
    Array.from({ length: tileCountY }, (_, ty) =>
      Array.from({ length: tileCountX }, async (__, tx) => {
        const url = TERRAIN_TILE_URL
          ...
          .replace("{x}", String(tileX0 + tx))
          .replace("{y}", String(tileY0 + ty));
        const response = await fetch(url);
        if (!response.ok) throw new Error(`terrain tile ${zoom}/${tileX0 + tx}/${tileY0 + ty}: HTTP ${response.status}`);
        ...
        // Blit tile pixels into the correct offset of the stitched array.
        const rowStart = ty * TILE_SIZE;
        const colStart = tx * TILE_SIZE;
        for (let py = 0; py < TILE_SIZE; py++) {
          for (let px = 0; px < TILE_SIZE; px++) {
            const rgbaOffset = (py * TILE_SIZE + px) * 4;
            const gridOffset = (rowStart + py) * cellsX + (colStart + px);
            data[gridOffset] = decodeTerrarium(rgba[rgbaOffset], rgba[rgbaOffset + 1], rgba[rgbaOffset + 2]);
          }
        }
      })
    ).flat()
