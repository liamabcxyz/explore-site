...
export function computeViewshed({
  ...
  for (let ringIndex = 0; ringIndex < numRings; ringIndex++) {
    ...
    for (let sectorIndex = 0; sectorIndex < numSectors; sectorIndex++) {
      ...
      // Stash the sample observer's lat/lng in properties so downstream
      // callers (LaunchPointControl.jsx's top-spots picker) can place
      // markers at exactly the point this cell's score was measured for,
      // instead of averaging the sector's corners back out — the sample
      // point is where the physics is honest, the corners are just the
      // painted region.
      const { lat: sampleLat, lng: sampleLng } = projector.toLatLng(observer.x, observer.y);
