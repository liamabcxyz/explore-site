...
function buildingDisplayName(properties) {
  ...
  if (typeof names === "string") {
    try {
      ...
      return buildingDisplayName({ names: parsed });
    } catch {
  ...
  if (names && typeof names === "object") {
    ...
    if (Array.isArray(common) && common[0]?.value) return String(common[0].value);
...
export function normalizeBuilding(properties, footprint) {
  ...
  const name = buildingDisplayName(properties);

  if (typeof properties.height === "number") {
    const confidence = HEIGHT_SOURCE_CONFIDENCE[properties.heightSource] ?? "high";
    return { footprint, base, height: properties.height, source: "height", confidence, name };
  }
